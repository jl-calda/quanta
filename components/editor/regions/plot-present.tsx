...
export interface PlotFigureProps {
  result: PlotResult;
  region: Pick<PlotRegion, "kind" | "x" | "y" | "xVar" | "yVar">;
...
function VectorFigure({ result, region, height }: { result: PlotResult; region: PlotFigureProps["region"]; height?: number }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${height ?? H}`} style={{ display: "block" }} role="img" aria-label="Vector field">
      ...
      <XYFrame sx={sx} sy={sy} xticks={xticks} yticks={yticks} xLabel={region.x?.label || region.xVar || "x"} yLabel={region.y?.label || region.yVar || "y"} xUnit={result.xUnit} yUnit={result.yUnit} height={height ?? H} />
      {vectors.map((v, i) => {
        const px = sx(v.x);
        const py = sy(v.y);
