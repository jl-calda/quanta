 ...
 * the signed-in user may see. The reads are wrapped in React `cache()` so a
 * layout and the page it wraps share one result per request (no double-fetch).
 */
...
export const getMyWorkspaces = cache(async function getMyWorkspaces(): Promise<
...
export const getActiveWorkspace = cache(
  async function getActiveWorkspace(): Promise<MembershipWithWorkspace | null> {
